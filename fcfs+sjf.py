...
def sjf(processes):
    ...
    while completed < n:
        ...
        available = [(i, p) for i, p in enumerate(processes) if p[1] <= current_time and not is_completed[i]]
        
        if available:
            # choose process with shortest burst time
            i, (pid, arrival, burst) = min(available, key=lambda x: x[1][2])
            start_time = current_time
            waiting_time[i] = start_time - arrival
            completion_time[i] = start_time + burst
            turnaround_time[i] = completion_time[i] - arrival
...
def main():
    while True:
        print("\n" + "="*60)
        print("CPU Scheduling Algorithms (Non-Preemptive)")
        print("="*60)
        ...
        try:
            ...
            if choice == 'no':
                print("\nThank you for using the CPU Scheduler. Goodbye!")
                break
